...
interface GameCardProps {
  ...
  onSave?: (result: GeneratedNumbers) => void;
}
...
export const GameCard: React.FC<GameCardProps> = ({ config, onSave }) => {
  ...
  const handleShare = async () => {
    if (!result) return;
    const shareText = `【發財靈籤】我在 ${config.name} 抽到了發財號碼：\n` +
      `A區：${result.zoneA.join(', ')}` +
      (result.zoneB ? `\nB區：${result.zoneB.join(', ')}` : '') +
      `\n祝大家財源廣進！💰✨`;
