...
傳統民俗儀式感與現代前端技術結合的樂透選號網頁應用（目前 **v1.6.0**）。選號結果**僅供娛樂參考**，不會提高中獎機率。
...
- 時辰真實表：查今日或指定日的十二時辰、台灣真太陽時（台北／台中／高雄經度）、農曆干支生肖、黃黑道十二天神，以及各時辰值神的傳統宜忌考量（職司、宜、忌與簡述）。宜忌隨值神而變，與黃曆「宜／忌」欄不是同一套。表格依寬度完整展開，此刻列以金線標示。僅供民俗計時參考，與選號、開獎無關
...
- `lunar-typescript`：農曆、干支與黃黑道輪值；十二天神宜忌考量為專案內建通書常見說法
...
- `src/lib/`：不放回抽號、分享、歷史紀錄、求籤進度、音效、時辰與真太陽時、農曆、黃黑道與十二天神宜忌
...
本專案提供的選號功能純屬娛樂用途，結果僅供參考。時辰、真太陽時、黃黑道與宜忌為民俗查閱，各家黃曆、通書可能不同，不會改變中獎機率。請理性購買樂透，切勿過度投注。
